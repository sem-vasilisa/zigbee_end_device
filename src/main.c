...
/* Parametry sterowane build-time (Kconfig aplikacji). Fallback na wypadek
 * builda bez pliku Kconfig w katalogu aplikacji - wartosci jak wczesniej. */
#ifndef CONFIG_ZB_LONG_POLL_INTERVAL_S
#define CONFIG_ZB_LONG_POLL_INTERVAL_S 30
#endif
#ifndef CONFIG_ZB_REPORT_PERIOD_S
#define CONFIG_ZB_REPORT_PERIOD_S 60
#endif
#define ZB_LONG_POLL_INTERVAL_MS (CONFIG_ZB_LONG_POLL_INTERVAL_S * 1000)
#define ZB_REPORT_PERIOD_MS      (CONFIG_ZB_REPORT_PERIOD_S * 1000)
...
static void send_test_report(zb_uint8_t param)
{
    ...
    if (!bufid) {
        ...
        ZB_SCHEDULE_APP_ALARM(send_test_report, 0, ZB_MILLISECONDS_TO_BEACON_INTERVAL(ZB_REPORT_PERIOD_MS));
        return;
    }
    ...
    ZB_SCHEDULE_APP_ALARM(send_test_report, 0, ZB_MILLISECONDS_TO_BEACON_INTERVAL(ZB_REPORT_PERIOD_MS)); /* domyslnie 1min - function repeats every minute */
}

int main(void){
    // LOG_INF("Starting Zigbee Light Bulb (Sleepy End Device)");
    // gpio_pin_configure_dt(&led, GPIO_OUTPUT_INACTIVE); /* configure led gpio as output and force it off */
    ...
    zb_set_keepalive_timeout(ZB_MILLISECONDS_TO_BEACON_INTERVAL(600000)); /* 10 mins - set a keepalive timeout - how often an end device contacts parent to say it's alive */
    zigbee_configure_sleepy_behavior(true); /* enable sleepy behavoir */

    zb_zdo_pim_set_long_poll_interval(ZB_LONG_POLL_INTERVAL_MS); /* domyslnie 30s - how often an end device wakes up to ask a parent about a new message  */
